...
def submit_form():
    ...
    def main():
        database = "C:\\Users\\HP_GAMING_15\\Desktop\\GUM\\db\\CriminalDetails.db"
    ...
    if __name__ == '__main__':
        create_connection("C:\\Users\\HP_GAMING_15\\Desktop\\GUM\\\\db\\CriminalDetails.db")
        main()
    ...
    def insertOrUpdate(Id,Name,Age,Gen,CN,Address,Cr):
        conn=sqlite3.connect("C:\\Users\\HP_GAMING_15\\Desktop\\GUM\\db\\CriminalDetails.db")
        cup= conn.cursor()
    ...
    if request.method == 'POST':
            if 'path' in request.files:
                ...
                img_filename='CAPTURED'+str(datetime.now())+'.jpg'
                # Save the captured image to upload folder
                cv2.imwrite(os.path.join(app.config['UPLOAD_FOLDER'], img_filename))
...
def search_img():
    ...
    def getProfile(id):
        conn = sqlite3.connect("db\\CriminalDetails.db")
        cmd = "SELECT * FROM People WHERE ID=" + str(id)
        cursor = conn.execute(cmd)
        profile = cursor.fetchone()  # Changed to fetch one record only
        conn.close()
        return profile
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            if profile is not None:
                text = f"Name: {profile[1]}, Age: {profile[2]}, Gender: {profile[3]}, Contact: {profile[4]}, Address: {profile[5]}"
                cv2.putText(image, text, (x, y - 10), fontface, fontscale, fontcolor, 2)
            else:
                flag = 1
...
#                 cv2.putText(image,"Name : "+str(profile[1]),(x+w,y+h+20),fontface, fontscale, fontcolor)
#                 cv2.putText(image,"Age : "+str(profile[2]),(x+w,y+h+45),fontface, fontscale, fontcolor)
#                 cv2.putText(image,"Gender : "+str(profile[3]),(x+w,y+h+75),fontface, fontscale, fontcolor)
#                 cv2.putText(image,"Contact no. : "+str(profile[4]),(x+w,y+h+180),fontface, fontscale, fontcolor)
#                 cv2.putText(image,"Address  : "+str(profile[5]),(x+w,y+h+225),fontface, fontscale, fontcolor)
#                 cv2.putText(image,"Criminal Records : "+str(profile[6]),(x,y+h+270),fontface, fontscale, fontcolor)
